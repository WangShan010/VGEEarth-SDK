...
const SceneUtils = {
    ...
    trueNorth() {
        const viewer = getMainViewer();
        // 相机的经纬度
        let camPos = Cesium.Cartographic.fromCartesian(viewer.camera.position,
            Cesium.Ellipsoid.WGS84, new Cesium.Cartographic());
        // 视野中心点
        let result = viewer.scene.pickPosition(
            new Cesium.Cartesian2(viewer.canvas.clientWidth / 2,
                viewer.canvas.clientHeight / 2));
        let newResult = Cesium.Cartographic.fromCartesian(result,
            Cesium.Ellipsoid.WGS84, new Cesium.Cartographic());
        // 相机高度更换为视野高度
        newResult.height = camPos.height;
        let dest = Cesium.Cartographic.toCartesian(newResult, Cesium.Ellipsoid.WGS84,
