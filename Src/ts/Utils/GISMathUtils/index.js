 ...
 * @param {*} delta
 */
function TwoPointsOverlay(pt1X, pt1Y, pt2X, pt2Y, delta) {
...
const GISMathUtils = {
     ...
     * @param diff  纬度值
     */
    lat_to_meter: (diff) => diff / 0.000000157891,
    lon_to_meter: (diff, lat) => diff / 0.000000156785 * Math.cos(lat),
    meter_to_lat: (m) => m * 0.000000157891,
    meter_to_long: (m, lat) => m * 0.000000156785 / Math.cos(lat),
    /**
     ...
     * @param result
     */
    pointProjectLine: function (ptX, ptY, L1X, L1Y, L2X, L2Y, result) {
     ...
     * @returns
     */
    TwoPointsDistance: (pt1X, pt1Y, pt2X, pt2Y) => Math.sqrt((pt1X - pt2X) * (pt1X - pt2X) + (pt1Y - pt2Y) * (pt1Y - pt2Y)),
    /**
     ...
     * @param radius
     */
    polarPoint: function (centerX, centerY, vector, radius) {
     ...
     * @returns
     */
    deltaAngle: function (direct1, direct2) {
     ...
     * @constructor
     */
    InterpolateLineLonlat: function (start, end) {
        if (start && end) {
        }
        else {
            return null;
        ...
        let interval = Math.sqrt(Math.pow((end.lon - start.lon), 2) + Math.pow((end.lat - start.lat), 2));
     ...
     * @returns
     */
    computeLineNormal: function (lonStart, latStart, lonEnd, latEnd, normal) {
    ...
    degree2rad: function (val) {
        return val * Math.PI / 180.0;
    },
    rad2degree: function (val) {
     ...
     */
    calculationIntersection: function (start, end, intersection) {
        if (intersection[0]) {
    ...
    // 空间三点，计算平面角度
    calculateTriangle: function (positions) {
        // 空间 三点 求角度，先转为二维
