...
function createNavigation(viewer: Viewer) {
    ...
    let options = {
        // 默认视角位置
        defaultResetView: new Cesium.Cartographic(
            Cesium.Math.toRadians(APPConfig.homeView.longitude || 110),
            Cesium.Math.toRadians(APPConfig.homeView.latitude || 40),
    ...
    if (APPConfig.homeView.headingRadians || APPConfig.homeView.pitchRadians || APPConfig.homeView.rollRadians) {
        options.defaultHeadingPitchRoll = new Cesium.HeadingPitchRoll(
