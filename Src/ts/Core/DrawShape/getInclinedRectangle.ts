...
function getInclinedRectangle(start: Cartesian3, end: Cartesian3, offset: Cartesian3, cartesian1: Cartesian3, cartesian2: Cartesian3) {
    ...
    let startX = GISMathUtils.lon_to_meter(cartographicStart.longitude - cx,
        cartographicStart.latitude);
    let startY = GISMathUtils.lat_to_meter(cartographicStart.latitude - cy);

    let endX = GISMathUtils.lon_to_meter(cartographicEnd.longitude - cx,
        cartographicEnd.latitude);
    let endY = GISMathUtils.lat_to_meter(cartographicEnd.latitude - cy);

    let offsetX = GISMathUtils.lon_to_meter(cartographicOffset.longitude - cx,
        cartographicOffset.latitude);
