...
const commitEndCallBack = (coordinateType, endCallback, ps) => {
...
class DrawShape {
    ...
    // 画点函数
    drawPoint({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
    ...
    ;
    // 画线函数
    drawLine({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
    ...
    ;
    // 画多折线
    drawPolyLine({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
    ...
    ;
    // 画角度
    drawTriangle({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
        that.errCallback = errCallback;
        let returnPositions = []; // 多折线坐标数组
    ...
    ;
    // 画多边形
    drawPolygon({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
        ...
        let returnPosition = []; // 诡异的bug，数组的值会发生跳动
        // 设置左键单击拾取坐标事件
        handler.setInputAction((event) => {
            // 获得鼠标点击位置的坐标
            let earthPosition = that.viewer.scene.pickPosition(event.position);
    ...
    ;
    // 画圆
    drawCircle({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
        ...
        handler.setInputAction((event) => {
            ...
            if (Cesium.defined(newPosition)) {
                if (circleCenter) {
                    let cartographic0 = that.viewer.scene.globe.ellipsoid.cartesianToCartographic(circleCenter);
                    let cartographic1 = that.viewer.scene.globe.ellipsoid.cartesianToCartographic(newPosition);
                    let geodesic = new Cesium.EllipsoidGeodesic(cartographic0, cartographic1);
     ...
     * @param errCallback
     */
    drawRectangle({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
                    ...
                    if (!that.drawEntities) {
                        ...
                        that.drawEntities = that.dataSourceToo.entities.add({
                            polygon: {
                                ...
                                material: new Cesium.ColorMaterialProperty(Cesium.Color.LIGHTSKYBLUE.withAlpha(0.3)),
     ...
     * @param errCallback
     */
    drawInclinedRectangle({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
    ...
    ;
    // 画高差
    drawHeightDistinct({ coordinateType, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
        ...
        handler.setInputAction((event) => {
            ...
            if (Cesium.defined(earthPosition)) {
                if (that.coordinates.length == 0) {
                    ...
                    that.dataSourceToo.entities.add(EntityFactory.PointLabelEntity(new Cesium.CallbackProperty(() => {
                    ...
                    }, false), new Cesium.CallbackProperty(() => GISMathUtils.getHeight(that.coordinates) + '米', false)));
                }
                else {
                    commitEndCallBack(coordinateType, endCallback, that.coordinates);
                    that.drawShapeEnd();
     ...
     * @param callback   返回值
     */
    drawYPlan({ position, normal, dimensions, endCallback, moveCallback, errCallback }) {
        let that = this;
        let handler = that.drawShapeStart();
        that.endCallback = endCallback;
        that.moveCallback = moveCallback;
