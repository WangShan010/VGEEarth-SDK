...
class RunEntityController {
    ...
    entity: Entity;
    lineGeoJson: any;  //没找到其具体类型，用any
    speed: number;
    loop: boolean;
    playing: boolean;
    rawPosition: PositionProperty | undefined;
    pathDataSources;
    clampToGround: boolean;
    playEndFunc: Function | null = null;
     ...
     * @param clampToGround
     */
    constructor(viewer: Viewer, entity: Entity, lineGeoJson: any, speed: number, loop = true, clampToGround = true) {
        this.viewer = viewer;
        this.entity = entity;
    ...
    play(playEndFunc: Function | null = null) {
    ...
    addPath() {
        let pathEntity = new Cesium.Entity({
            polyline: {
                positions: Cesium.Cartesian3.fromDegreesArray(this.lineGeoJson.geometry.coordinates.map((item: number[]) => [item[0], item[1]]).flat()),
                width: 12,
